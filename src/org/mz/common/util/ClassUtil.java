...
public class ClassUtil {
			...
			while (dirs.hasMoreElements()) {
				...
				if ("file".equals(protocol)) {
					...
					try {
								...
								if ((idx != -1) || recursive) {
									...
									if (name.endsWith(".class") && !entry.isDirectory()) {
										...
										try {
											...
											logger.error("扫描jar包中class失败，类找不到", e);
						...
						logger.error("扫描jar包中class失败", e);
					}
				}
			}
		} catch (IOException e) {
			logger.error("扫描包中class失败", e);
		}
		...
		for (File file : dirfiles) {
			...
			if (file.isDirectory()) {
				...
				try {
					...
					logger.error("扫描jar包中class失败, 找不到此类的.class文件", e);
				} catch (NoClassDefFoundError e) {
					logger.error("扫描jar包中class失败，类找不到", e);
				}
			}
	...
	public static void main(String[] args) {
		System.out.println(getClasses("bsh"));
	}
}
